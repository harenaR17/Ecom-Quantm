...
const CartSummary: React.FC<CartSummaryProps> = ({ subtotal, itemCount }) => {
  return (
    <div className="bg-green-50 rounded-lg p-6 shadow-sm sticky top-28">
      ...
      <div className="flex justify-between text-lg font-bold text-emerald-900 mb-6">
        <span>Subtotal</span>
        <span>€{subtotal.toFixed(2)}</span>
      </div>
      ...
      <button
        type="button"
        disabled
        className="w-full bg-emerald-300 text-white py-3 px-8 rounded-md font-medium cursor-not-allowed"
      >
